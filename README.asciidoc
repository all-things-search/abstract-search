...
abstract-search is a Java service wrapper for the search engine Elasticsearch.
...
As you can see, abstract-search focuses more on WHAT to search for than on HOW to search it and abstracts some complexity.

== Getting started

Only a few steps are necessary to get started with a standard configuration.
A complete code example can be found https://github.com/all-things-search/abstract-search-samples[here].
If you want to create a Spring Boot application, you can alternatively use the https://github.com/all-things-search/abstract-search-starter[abstract-search Starter].

For more information, please see following https://picturesafe-search.io/docs/[documentation].

=== Start Elasticsearch

abstract-search requires a running Elasticsearch server from version 7.x.
...
Add the current version of the abstract-search library to your project.
...
With implementations of the abstract-search `Expression`-Interface complex terms of different search conditions can be easily defined.
